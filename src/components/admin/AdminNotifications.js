...
export default function AdminNotifications() {
  ...
  const load = async () => {
    try {
      ...
      console.error('Lỗi tải danh sách thông báo (admin):', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMarkRead = async (id) => {
    try {
      ...
      console.error('Lỗi cập nhật thông báo (admin):', e);
    }
  };
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Danh sách đầy đủ thông báo hệ thống dành cho quản trị.</p>
        </div>
      </div>
          ...
          <ul className="divide-y divide-gray-100">
            {notifications.map((n) => (
              ...
              >
                <div className="pt-1">
                  <span
                    className={`inline-block w-2.5 h-2.5 rounded-full ${n.readFlag ? 'bg-gray-300' : 'bg-indigo-500'}`}
                  />
                </div>
